...
from src.helper.noise_function import NoiseFunction
...
class TestWeigh(unittest.TestCase):
    def test_weigh_without_weights(self):
        ...
        expected_result = (result_f1 + result_f2) / (
            expected_weight_f1 + expected_weight_f2
        )

        result = NoiseFunction.weigh(x, y, [function1, function2])

        self.assertEqual(
            expected_result, result, "Function noise output does not match."
        )
    ...
    def test_weigh_with_custom_weights(self):
        ...
        result = NoiseFunction.weigh(
            x, y, [function1, function2], [weigth_f1, weigth_f2]
        )
