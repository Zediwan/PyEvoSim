from code.simulation.entities import Animal
import pygame
import unittest
...
class TestAnimal(unittest.TestCase):
    @patch("code.simulation.world.tile.Tile", "code.simulation.dna.dna.DNA")
    def setUp(self, mock_tile, mock_dna) -> None:
        self.base_rect = pygame.Rect(0, 0, 1, 1)
        self.base_tile = mock_tile
        self.base_dna = mock_dna
        self.parent = Animal(tile=self.base_tile, rect=self.base_rect, dna=self.base_dna)
        self.child = Animal(tile=self.base_tile, rect=self.base_rect, dna=self.base_dna, parent=self.parent)
