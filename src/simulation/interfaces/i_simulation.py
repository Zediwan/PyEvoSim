...
class IPygameSimulation(ISimulation):
    def __init__(self, x: int, y: int, width: int, height: int, fps_limit = 0, background_color = pygame.Color("black")) -> None:
...
class IPygameMenuSimulation(IPygameSimulation):
    def __init__(self, x: int, y: int, width: int, height: int, fps_limit=0, background_color=pygame.Color("black"), starting_menu: pygame_menu.Menu = None, pause_menu: pygame_menu.Menu = None) -> None:
        super().__init__(x, y, width, height, fps_limit, background_color)
        
        if starting_menu is None:
            starting_menu = pygame_menu.Menu("Welcome", self.rect.width, self.rect.height)
